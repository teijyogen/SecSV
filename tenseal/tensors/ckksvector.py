...
class CKKSVector(AbstractTensor):
    ...
    def rotate_vector_inplace(self, steps):
        self.data.rotate_vector_inplace(steps, self.context().data.galois_keys())
        return self

    def rotate_vector(self, steps) -> "CKKSVector":
        result = self.data.rotate_vector(steps, self.context().data.galois_keys())
        return self._wrap(result)